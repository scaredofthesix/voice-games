...
- Run the customer retests of UAT-12 (Voice Maze Quest), UAT-13 (one-failure adaptive) and
  UAT-15 (bulk paste) in the final confirmation call, then record the results.
- Cut the MVP v3 SemVer release only after the confirmation call and the public demo video, so
  the release links both, and move `CHANGELOG [Unreleased]` into a dated section.
