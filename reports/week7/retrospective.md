...
- The custom-word paste was hardened per the live finding: a tab, and as a fallback a run of
  four or more spaces, separate the columns, so multi-word phrases survive (#144 follow-up).
  The 2026-07-17 follow-up clarified that the fallback must be exactly four spaces; the final
  candidate enforces that stricter rule and keeps invalid rows editable.
...
- **In progress:** cut the MVP v3 SemVer release after independent PR review and protected
  merge. The confirmation call and public demo video are complete; the release must link the
  demo and the Week 7 report.
